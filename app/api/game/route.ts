...
export async function GET(req: Request) {
  try {
    ...
    const cookie = fresh
      ? `ng_session=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=31536000${
    ...
    if (!code) {
      return reply({ ready: true }, 200, cookie);
    }

    if (!/^\d{6}$/.test(code)) {
      return reply(
        { error: '6桁のコードを入力してください' },
        400,
        cookie,
      );
    }
    ...
    for (let n = 0; n < 5; n++) {
      ...
      if (!row) {
        return reply({ error: 'ルームが見つかりません' }, 404, cookie);
      }
      ...
      if (
        ...
        !r.spectators?.some((p) => p.id === id)
      ) {
        return reply(
          {
            error: 'プレイヤー名を入力して参加してください',
            join: true,
          },
          403,
          cookie,
      ...
      if (new URL(req.url).searchParams.has('hover')) {
        return reply(
          { hover: await readHover(r) },
          200,
          cookie,
        );
      }
...
export async function POST(req: Request) {
  try {
    ...
    if (
      origin &&
      forwardedHost &&
      !origin.includes(forwardedHost) &&
      !origin.includes(requestHost ?? '')
    ) {
      return reply(
        { error: 'このページから操作してください' },
        403,
      );
    }
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      if (r.commands?.includes(key) && existing) {
        return reply(
          view(r, id, await presenceFor(code)),
          200,
          cookie,
        );
      }
      ...
      if (body.type === 'join') {
        try {
          joinRoom(r, id, String(body.name ?? ''));
        } catch (e) {
          return reply(
